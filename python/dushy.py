...
class Duschy(threading.Thread):
				...
				for cat_t in categ_arr:
					...
					if cat_t not in self.category_links_inserted:
						...
						if ci > 0:
							...
							sql = "INSERT INTO categories (name, description, created_at, updated_at, id_upper) VALUES ("
	...
	def dbInsert(self, name, desc, img, cat_id, code):
		...
		cur = self.conn.cursor()
		sql = "INSERT INTO items (name, description, image_path, category_name, code, price, weight, count, created_at, updated_at) VALUES ("
		sql += "'"+name +"', '"+desc+"', '"+img+"', 'cat', '"+code+"', 1, 1, 1, now(), now())"
		cur.execute(sql)
		self.conn.commit()
	...
	def getItemsDataFromDuschy(self):
		if self.item_links:
			...
			try:
				...
				urllib.request.urlretrieve(imagesrc, '/home/error/kod/hiveware/storage/shop/duschi/'+image_name)

				item_cont = soup.find('div', {"id":"viewProduct"})
				item_name = str(item_cont.select('span#viewProductHead > h2')[0]).lstrip('<h2>').rstrip('</h2>')
				item_desc = str(item_cont.select('span#viewProductDescription')[0])
